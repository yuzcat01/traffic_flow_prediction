...
- 提供 PyQt GUI：数据预览、实验训练、模型管理、在线推理、路线规划、拥堵预警、事件模拟、结果分析
- 提供交通应用原型：基于预测流量进行节点级路线推荐、全网拥堵预警和事件影响模拟
...
- `docs/application_decision.md`：路线规划、拥堵预警与事件模拟应用模块说明
...
6. 在“在线推理”页面查看预测效果
7. 在“路线规划”页面选择起终点和推荐策略，查看候选路线、可达性和路网高亮
8. 在“拥堵预警”页面查看未来时刻全网高风险节点
9. 在“事件模拟”页面设置事故/管制节点，比较事件前后的推荐路线变化
10. 在“结果分析”页面对比不同模型、不同建图方式的结果
...
项目新增三个应用页面，用于展示交通流量预测模型在真实业务中的使用方式。页面会复用当前加载模型的在线推理结果，将未来节点流量转换为拥堵指数，并结合图结构文件中的路网边距离生成路线建议、预警摘要和事件影响对比。

当前支持：

- 历史测试样本回放，模拟实时路网流量输入
- 路线规划：起点节点、终点节点、预测步长和推荐策略选择，并提供“选择最远预测步长”快捷按钮
- 支持起终点可达性查询，显示是否连通、最少连接跳数和连接路径
- 三类推荐策略：距离最短、优先避堵、综合最优
- 根据路网连接关系绘制整体拓扑预览
- 支持多条候选路线对比，并在路网图中高亮当前选中路线
- 提供“最短路线 vs 当前路线”对比表和路线推荐原因解释
- 拥堵预警：全网未来拥堵风险 Top-K 节点识别、严重风险节点统计和预警建议
- 事件模拟：对指定节点施加流量改变倍数，支持放大或缩小流量，并比较事件前后路线、拥堵指标和推荐原因变化
- 推荐路线节点级预测流量、参考流量和拥堵等级展示
- 路线推荐报告 CSV 导出

说明：路线图表中的“参考流量”来自该节点历史流量的 85% 分位数，用作拥堵指数的基准值；它不是道路容量，也不是外部交通平台给出的限流阈值。PeMS04 默认数据集提供的是传感器节点拓扑和边距离，不包含真实经纬度、道路名称、限速或车道数。因此该功能定位为“基于传感器节点拓扑的路线推荐原型”，适合用于课程设计、论文和答辩中展示预测模型如何进入交通辅助决策环节。
...
- 路线规划：选择起终点、查询可达性、对比多条候选路线并高亮路网路径
- 拥堵预警：按预测步长查看全网高风险节点、预警指标和风险柱状图
- 事件模拟：设置事件节点和流量改变倍数，比较事件前后路线推荐变化
