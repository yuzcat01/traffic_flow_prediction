...
class ApplicationPage(QWidget):
    ...
    def _init_ui(self):
        ...
        eyebrow = QLabel("Application Decision")
        eyebrow.setObjectName("HeroEyebrow")
        title = QLabel("预测驱动的拥堵预警与路线推荐")
        ...
        for text in ["路网拓扑预览", "多候选路线", "图中路线高亮"]:
            badge = QLabel(text)
    ...
    def set_predictor(self, predictor, current_model_row=None):
        ...
        try:
            ...
            self.network_preview = None
            self._reset_view()
            self.text_route.setPlainText(f"应用决策模块初始化失败：{e}")
    ...
    def _reset_view(self):
        ...
        self.text_route.setPlainText("请先在“模型管理”页面加载模型，然后在本页生成拥堵预警与路线推荐。")
    ...
    def _update_selected_route_view(self, recommendation):
        ...
        lines = [
            ...
            f"预测步长：h{recommendation['horizon_idx'] + 1}，策略：{self.combo_strategy.currentText()}，拥堵权重：{recommendation['alpha']:.2f}",
        ...
        self.label_summary.setText(
            ...
            f"当前选中候选路线 {recommendation.get('route_rank', 1)}，经过 {recommendation['node_count']} 个节点。\n"
    ...
    def _draw_route_charts(self, route_nodes):
        ...
        self.canvas_route.ax_flow.plot(x, baselines, marker="s", color="#64748b", linestyle="--", label="参考流量")
